...
On my macOS computers, Emacs is pretty slow compared to other
editors. So slow that writing this Org document on my 11 year old
MacBook Pro with 16 GB RAM is painful. (Something producing garbage like
mad; =org-indent-mode= contributes to that, but is not alone.)

That's ridiculous. I used that same MBPro for C++ development, and it is
plenty fast enough for that.

So I thought it might be an idea to make Emacs use more than one core.

I'm doing this for my own entertainment. I haven't talked with the
current maintainers about plans of using MPS in Emacs.

* Caveats

This is based on my local Emacs, which is different from mainstream
Emacs. My local Emacs uses CL packages, doesn't have obarrays, doesn't
support pure space, does not support shorthands and probably some other
stuff.

In addition, I'm using macOS only, so it's unlikely to compile or run on
other systems OOTB. It should not be too hard to port, though.
