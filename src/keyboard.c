			 ...
			 Lisp_Object *window_or_frame, Lisp_Object *posn)
{
  ...
  /* WINDOW_OR_FRAME is the window where (MX, MY) is located: Its frame
     N is certainly not the frame we can drag because the position is
     inside.  It can be an arbitrary frame whose root is R.  */
  struct frame *n = WINDOW_XFRAME (XWINDOW (*window_or_frame));
  struct frame *f = NULL;
  Lisp_Object tail, frame;
  int ix, iy = 0;
